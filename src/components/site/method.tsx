  ...
  return (
    <section id="method" className="rule-top bg-secondary/60">
      <div className="mx-auto w-full max-w-6xl px-5 py-16 sm:px-8 sm:py-24">
        ...
        <ol className="mt-12 grid gap-px overflow-hidden rounded-lg border border-hairline bg-hairline sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((s) => (
            <li key={s.n} className="bg-card p-6 sm:p-7">
              <span className="font-mono text-xs text-signal">{s.n}</span>
              <h3 className="mt-4 text-lg tracking-tight sm:text-xl">{s.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{s.body}</p>
            </li>
          ))}
        </ol>

        <div className="mt-14 rounded-xl border border-hairline bg-card p-6 shadow-card sm:p-10">
          <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr] lg:gap-14">
            <div>
              <p className="eyebrow">Integrity</p>
              <p className="mt-4 font-display text-2xl leading-[1.15] sm:text-[2.1rem]">
